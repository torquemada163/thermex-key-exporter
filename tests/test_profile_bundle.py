...
def test_bundled_profile_is_loaded_from_python_package_data(tmp_path, monkeypatch) -> None:
    package_root = tmp_path / "thermex_key_exporter"
    path = package_root / "data" / "thermex-profile.json"
    write_profile_bundle(profile(), path)
    monkeypatch.delenv("THERMEX_PROFILE_PATH", raising=False)
    monkeypatch.delattr(profile_bundle.sys, "_MEIPASS", raising=False)
    monkeypatch.setattr(profile_bundle, "files", lambda _package: package_root)

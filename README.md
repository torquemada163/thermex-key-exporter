...
В архивах Windows и Linux находятся две самостоятельные версии приложения:
GUI, рекомендуемый для большинства людей, и CLI для терминала. На macOS один
GUI-bundle также предоставляет CLI через команду рядом с ним.

| Платформа | GUI | CLI |
| --- | --- | --- |
| Windows x64 | `ThermexKeyExporter\ThermexKeyExporter.exe` | `thermex-key-exporter\thermex-key-exporter.exe` |
| macOS ARM64 | `ThermexKeyExporter.app` | `thermex-key-exporter` |
...
macOS держите `thermex-key-exporter` рядом с `ThermexKeyExporter.app`; не
изменяйте содержимое `.app`-bundle и не выносите из него исполняемый файл.
...
2. Для CLI выполните одну из команд:
   ...
   macOS: ./thermex-key-exporter export --output thermex-localtuya.json
   ```
...
После этого CLI запускается командой `./thermex-key-exporter` из той же папки,
без перехода внутрь bundle. Не отключайте Gatekeeper и не снимайте карантин с
отдельных файлов внутри `.app`.
